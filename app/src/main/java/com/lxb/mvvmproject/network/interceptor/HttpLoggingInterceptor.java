    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logHeaders) {
            if (hasRequestBody) {
                ...
                if (requestBody.contentType() != null) {
                    LogUtil.i("Content-Type: " + requestBody.contentType());
                }
            ...
            for (int i = 0, count = headers.size(); i < count; i++) {
                ...
                if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
                    LogUtil.i(name + ": " + headers.value(i));
                }
            }
            ...
            if (!logBody || !hasRequestBody) {
                ...
                if (isPlaintext(buffer)) {
                    LogUtil.i(buffer.readString(charset));
                    LogUtil.i("END " + request.method()
                            + " (" + requestBody.contentLength() + "-byte body)");
                } else {
                    LogUtil.i("END " + request.method() + " (binary "
        ...
        if (logHeaders) {
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                LogUtil.i("END HTTP (" + buffer.size() + "-byte body)");
            }
        }
