...
public final class CustomActivityOnCrash {
        ...
        try {
            if (context == null) {
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    ...
                    //我们定义了一个默认异常处理程序，它可以执行我们想要的操作，这样就可以从Crashlytics/ACRA调用它
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        @Override
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            if (config.isEnabled()) {
                                Log.e(TAG, "应用程序崩溃，执行CustomActivityOnCrash的UncaughtExceptionHandler", throwable);
                ...
                Log.i(TAG, "CustomActivityOnCrash已安装。");
            }
        } catch (Throwable t) {
            Log.e(TAG, "安装CustomActivityOnCrash时发生未知错误，可能未正确初始化。如果需要，请将此报告为bug。", t);
        }
    }
     ...
     * 给定一个意图，从其中返回额外的活动日志。
     *
     * @param intent 意图。不能为空.
     * @return 活动日志，如果没有提供，则返回null
     */
    @Nullable
    ...
    /**
     * 给定一个意图，返回多个错误细节，包括来自该意图的额外堆栈跟踪.
    ...
    public static String getAllErrorDetailsFromIntent(@NonNull Context context, @NonNull Intent intent) {
        ...
        errorDetails += "app版本: " + versionName + " \n";
        if (buildDateAsString != null) {
            errorDetails += "Build date: " + buildDateAsString + " \n";
        }
        errorDetails += "出错时间: " + dateFormat.format(currentDate) + " \n";
        ...
        errorDetails += "设备名称: " + getDeviceModelName() + " \n \n";
        errorDetails += "堆叠追踪:  \n";
